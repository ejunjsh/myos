...
hd60M.img:
	...
	bximage -hd=80M -imgmode=flat -mode=create -q hd80M.img
	fdisk -iy hd80M.img

$(BUILD_DIR)/mbr.bin:
	$(AS) -I boot/include -o $(BUILD_DIR)/mbr.bin boot/mbr.S 

$(BUILD_DIR)/loader.bin:
	$(AS) -I boot/include -o $(BUILD_DIR)/loader.bin boot/loader.S 
...
hd: hd60M.img $(BUILD_DIR)/mbr.bin $(BUILD_DIR)/loader.bin
	dd if=$(BUILD_DIR)/mbr.bin of=hd60M.img bs=512 count=1 conv=notrunc \
    && dd if=$(BUILD_DIR)/loader.bin of=hd60M.img bs=512 seek=2 count=4 conv=notrunc \
	&& dd if=$(BUILD_DIR)/kernel.bin \
           of=hd60M.img \
           bs=512 count=200 seek=9 conv=notrunc

clean:
	rm -rf $(BUILD_DIR) hd60M.img hd80M.img
